...
def main():
    ...
    parser.add_argument('-solver', '--solverfile', help='Prototxt solver file for caffe training',
                        required=False)
    parser.add_argument('-datadir', '--datadir', '--dataDir', help='Data directory where the dataset is located',
                        required=True)
    parser.add_argument('-outputdir', '--outputdir', '--modelDir', help='Output directory for checkpoints and models',
                        required=True)
    parser.add_argument('-logdir', '--logdir', '--logDir', help='Log directory', required=True)
    ...
    with cwd(data_path):
        ...
        if rank == 0 and os.path.exists(stdout_file):
