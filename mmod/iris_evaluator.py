...
class ClassificationEvaluator(Evaluator):
    def add_predictions(self, predictions, targets):
        ...
        Args:
            ...
            targets: the golden truths. Shape (N,) or (N, num_class)
        """
        assert len(predictions) == len(targets)
        if len(targets.shape) > 1: # Multi-label
            pass
        else:
    ...
    def get_report(self):
        ...
        return {'accuracy': float(self.correct_num) / self.total_num if self.total_num else 0.0}

    def reset(self):
        super(ClassificationEvaluator, self).reset()
        self.correct_num = 0
        self.total_num = 0
...
class MulticlassClassificationEvaluator(Evaluator):
    ...
    def reset(self):
        super(MulticlassClassificationEvaluator, self).reset()
...
class MultilabelClassificationEvaluator(Evaluator):
    ...
    def reset(self):
        super(MultilabelClassificationEvaluator, self).reset()
        self.correct_num = 0
        self.ap = 0
        self.total_num = 0
...
class ObjectDetectionSingleIOUEvaluator(Evaluator):
    ...
    def reset(self):
        ...
        super(ObjectDetectionSingleIOUEvaluator, self).reset()
...
class ObjectDetectionEvaluator(Evaluator):
    ...
    def reset(self):
        for evaluator in self.evaluators:
            evaluator.reset()
        super(ObjectDetectionEvaluator, self).reset()
