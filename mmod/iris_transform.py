...
def _adjust_bboxes(bboxes):
    new_bboxes = []
    for bbox in bboxes:
        adjusted_bbox = adjust_box(bbox[1], bbox[2], bbox[3], bbox[4])
        if adjusted_bbox:
            new_bboxes.append([bbox[0], adjusted_bbox[0], adjusted_bbox[1], adjusted_bbox[2], adjusted_bbox[3]])
    return new_bboxes
...
class YOLOV2PostProcess(object):
    ...
    def __call__(self, img, target):
        ...
        if self.for_training:
            yolo_target = np.zeros(shape=(len(target), 5), dtype=float)
...
class YoloV2InferenceTransform(Transform):
    def __init__(self, input_size):
        self.transforms = [ODImageTransform(FixedAreaTransform(input_size)),
                           YOLOV2PostProcess(for_training=False)]
...
class YoloV2TrainingTransform(Transform):
    def __init__(self, input_size):
        self.transforms = [ODRandomExpand((0.5, 0.5, 0.5)),
                           ...
                           ODRandomHorizontalFlip(),
                           ODImageTransform(torchvision.transforms.ColorJitter(hue=0.05, saturation=0.05)),
                           YOLOV2PostProcess(for_training=True)]