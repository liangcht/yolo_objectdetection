...
class Yolo(nn.Module):
    ...
    def __init__(self, backbone_model, num_classes=20, num_extra_convs=3, num_anchors=5):
        ...
        :param num_extra_convs: number of extra blocks of Conv2D
                            + BtachNorm + LeakyRelu to add to the featurizer
        :param num_anchors: int, number of anchors to use for
        """
        super(Yolo, self).__init__()
        self.backbone = backbone_model
        self.extra_convs = nn.Sequential(*_extra_conv_layers(self.backbone.out_planes_dim, num_extra_convs))
        ...
        self.regressor = nn.Sequential(OrderedDict([
                    ("last_conv", nn.Conv2d(self.backbone.out_planes_dim, regressor_dim, kernel_size=1,
                                            padding=0, bias=True))
        ]))
